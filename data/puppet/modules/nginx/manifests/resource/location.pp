...
define nginx::resource::location(
  ...
  $try_files            = undef,
  $loc_alias_erb        = $nginx::params::nx_vhost_loc_alias_erb,
  $loc_directory_erb    = $nginx::params::nx_vhost_loc_directory_erb,
  $loc_proxy_erb        = $nginx::params::nx_vhost_loc_proxy_erb,
  $loc_stub_status_erb  = $nginx::params::nx_vhost_loc_stub_status_erb,
  ...
  if ($proxy != undef) {
    $content_real = template($loc_proxy_erb)
  } elsif ($location_alias != undef) {
    $content_real = template($loc_alias_erb)
  } elsif ($stub_status != undef) {
    $content_real = template($loc_stub_status_erb)
  } else {
    $content_real = template($loc_directory_erb)
  }
