...
define nginx::resource::vhost(
  ...
  $try_files              = undef,
  $vhost_footer_erb       = $nginx::params::nx_vhost_footer_erb,
  $vhost_header_erb       = $nginx::params::nx_vhost_header_erb,
  $vhost_ssl_header_erb   = $nginx::params::nx_vhost_ssl_header_erb,
  ...
  if ($listen_port != $ssl_port) {
    file { "${nginx::config::nx_temp_dir}/nginx.d/${name}-001":
      ...
      content => template($vhost_header_erb),
  ...
  if ($listen_port != $ssl_port) {
    file { "${nginx::config::nx_temp_dir}/nginx.d/${name}-699":
      ...
      content => template($vhost_footer_erb),
      notify  => Class['nginx::service'],
    }
  }
  ...
  if ($ssl == 'true') {
    file { "${nginx::config::nx_temp_dir}/nginx.d/${name}-700-ssl":
      ...
      content => template($vhost_ssl_header_erb),
      notify => Class['nginx::service'],
    }
