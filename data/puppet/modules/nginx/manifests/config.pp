...
class nginx::config(
  ...
  $nginx_conf_erb      = $nginx::params::nx_nginx_conf_erb,
  $proxy_conf_erb      = $nginx::params::nx_proxy_conf_erb
  ...
  file { "${nginx::params::nx_conf_dir}/nginx.conf":
    ...
    content => template($nginx_conf_erb),
  }

  file { "${nginx::params::nx_conf_dir}/conf.d/proxy.conf":
    ensure  => file,
    content => template($proxy_conf_erb),
  }
